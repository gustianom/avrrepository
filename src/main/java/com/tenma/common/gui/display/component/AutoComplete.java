 ...
 * - support validator
 * <p/>
 * Version 0.3 (20.06.2015)
 * - Added reset button on autofilter popoup (default)
 * - Added reset button on field (optional) - using new constructor
 * - Added new method for resetButionEvent, require to override for catch reset button event
 * <p/>
 * <p/>
 * version 0.5
...
public abstract class AutoComplete extends CustomField implements Button.ClickListener {
    ...
    protected TextField lookupTextId = new TextField();
    ...
    protected AutoComplete(String param) {
        this.parm = param;
        initAutoComplete();
        lookupTextId.setValidationVisible(false);
        lookupTextId.setNullRepresentation("");
        ...
        lookupTextId.addTextChangeListener(new FieldEvents.TextChangeListener() {
            public void textChange(FieldEvents.TextChangeEvent event) {
                if (event.getText().length() > 0
                    ...
                    filterFld.focus();
                    filterFld.setCursorPosition(lookupTextId.getCursorPosition());
